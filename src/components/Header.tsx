...
export function Header({ title, back }: { title: string; back?: boolean }) {
  ...
  return (
    <header className="safe-top sticky top-0 z-30 border-b border-border bg-surface backdrop-blur-xl">
      <div className="mx-auto flex max-w-xl items-center gap-3 px-4 py-3.5">
        {back && (
          <button
            onClick={() => navigate(-1)}
            aria-label="Retour"
            className="-ml-1 flex h-8 w-8 items-center justify-center rounded-full text-slate-300 transition-transform active:scale-90 active:bg-surface-2"
          >
